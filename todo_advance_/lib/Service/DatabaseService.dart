...
import 'package:todo_advance_/Model/ToDoListModel.dart';
...
class DatabaseService {
  ...
  static createDatabase() async {
    database = openDatabase(join(await getDatabasesPath(), "todoDB.db"),
      ...
      await db.execute(
          ''' CREATE TABLE Task(id INT PRIMARY KEY,title TEXT,description TEXT,date TEXT)''');
    });
  }
  ...
  static Future deleteTask(ToDoModelClass obj) async {
    ...
    await localDB.delete("Task", where: "id = ?", whereArgs: [obj.id]);
  }

  static Future updateTask(ToDoModelClass obj) async {
    final localDB = await database;
    await localDB
        .update("Task", obj.todoMap(), where: "id = ?", whereArgs: [obj.id]);
  }
